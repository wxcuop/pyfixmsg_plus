...
class TestNetworkConnectionInitiator:
    ...
    async def test_initiator_disconnect(self):
        initiator = Initiator(host='localhost', port=8080)
        mock_writer = Mock()  # Use Mock instead of AsyncMock for synchronous methods
        mock_writer.is_closing.return_value = False
        mock_writer.wait_closed = AsyncMock()  # Only wait_closed needs to be async
        initiator.writer = mock_writer
        initiator.running = True
...
class TestNetworkConnectionAcceptor:
    ...
    async def test_acceptor_start_accepting(self):
        ...
        with patch('asyncio.start_server') as mock_start_server:
            mock_server = Mock()  # Use Mock instead of AsyncMock for synchronous attributes
            ...
            mock_server.close = Mock()  # Synchronous method
            mock_server.wait_closed = AsyncMock()  # Async method
    ...
    async def test_acceptor_disconnect(self):
        acceptor = Acceptor(host='0.0.0.0', port=8080)
        mock_server = Mock()  # Use Mock instead of AsyncMock for synchronous methods
        mock_server.close = Mock()  # Synchronous method
        mock_server.wait_closed = AsyncMock()  # Async method
    ...
    async def test_acceptor_set_transport(self):
        acceptor = Acceptor(host='0.0.0.0', port=8080)
        mock_reader = Mock()  # Use Mock for reader
        mock_writer = Mock()  # Use Mock for writer
        mock_writer.get_extra_info.return_value = ('127.0.0.1', 12345)
        
        # Mock the parent set_transport method since it's async
        with patch.object(acceptor.__class__.__bases__[0], 'set_transport', new_callable=AsyncMock) as mock_set_transport:
            await acceptor.set_transport(mock_reader, mock_writer)
            
            # Verify the parent method was called
            mock_set_transport.assert_called_once_with(mock_reader, mock_writer)
