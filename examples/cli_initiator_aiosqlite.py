...
if __name__ == "__main__":
    ...
    if not os.path.exists(initiator_config_file):
        ...
        default_cfg_writer.set('FIX', 'sender', 'BANZAI-AIO') 
        default_cfg_writer.set('FIX', 'target', 'EXEC-AIO') 
    ...
    if os.path.exists(db_file_init):
        try:
            ...
            logger.info(f"Removed previous aiosqlite initiator state file: {db_file_init}")
        except OSError as e:
            logger.error(f"Error removing aiosqlite initiator state file {db_file_init}: {e}")
